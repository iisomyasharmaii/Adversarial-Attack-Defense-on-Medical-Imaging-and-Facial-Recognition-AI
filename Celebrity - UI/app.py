...
load_Faces_model = load_model('Faces_model_somya') # Load model
class_names=['ben_afflek', 'elton_john', 'jerry_seinfeld', 'madonna', 'mindy_kaling', "somya_sharma"]
...
def predict():
    ...
    prob=load_Faces_model.predict(tf.expand_dims(img,axis=0))
...
def adversarial_pattern_FGSM(image, label):
    ...
    with tf.GradientTape() as tape:
        tape.watch(image)
        prediction = load_Faces_model(tf.expand_dims(image,axis=0))
...
def predictFGSM():
    ...
    #adversarial_image = FGSM(model, img, 0.04, 1, True) ## Adversarial Attack with target value of 1
    perturbations=perturbations_FGSM_generator(img,load_Faces_model)
    adversarial_image = img + perturbations *2

   
    prob=load_Faces_model.predict(tf.expand_dims(adversarial_image,axis=0))
    pred_acc=prob.max()*100
    pred_class=class_names[prob.argmax()]
    print(os.listdir())
...
def predictNLM():
    ...
    cv2.imwrite('static/denoised_image.jpg', denoised)

    denoise_image=load_and_prep_image('static/denoised_image.jpg')


    prob=load_Faces_model.predict(tf.expand_dims(denoise_image,axis=0))
    pred_acc=prob.max()*100
    pred_class=class_names[prob.argmax()]
